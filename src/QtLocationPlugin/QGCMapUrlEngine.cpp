...
UrlFactory::getTileURL(MapType type, int x, int y, int zoom, QNetworkAccessManager* networkManager)
{
    ...
    switch (type) {
        ...
        case BingMap:
        case BingSatellite:
        request.setRawHeader("Referrer", "https://www.amap.com/");
...
UrlFactory::_getURL(MapType type, int x, int y, int zoom, QNetworkAccessManager* networkManager)
{
    ...
    */
    case BingMap:
    {
        ...
        return QString("http://webst01.is.autonavi.com/appmaptile?style=6&x=%1&y=%2&z=%3").arg(x).arg(y).arg(zoom);
    ...
    {
        QString key = _tileXYToQuadKey(x, y, zoom);
        return QString("http://webst01.is.autonavi.com/appmaptile?style=7&x=%1&y=%2&z=%3").arg(x).arg(y).arg(zoom);
    ...
    case BingHybrid:
    {
        QString key = _tileXYToQuadKey(x, y, zoom);
        return QString("http://webst01.is.autonavi.com/appmaptile?style=7&x=%1&y=%2&z=%3").arg(x).arg(y).arg(zoom);
        //return QString("http://t0.tianditu.gov.cn/img_c/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=img&STYLE=default&TILEMATRIXSET=w&FORMAT=tiles&TILEMATRIX={%1}&TILEROW={%2}&TILECOL={%3}&tk=31fd2671aaadb8fefce1edd6af734ecc").arg(zoom).arg(y).arg(x);
